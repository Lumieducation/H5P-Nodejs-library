...
const H5pPackageValidator = require('../src/validator');
...
describe('validating H5P files', () => {
    ...
    beforeEach(async () => {
        ...
        translationService = new TranslationService(englishStrings, englishStrings);
    });
    ...
    it('rejects files not ending with .h5p', async () => {
        ...
        const validator = new H5pPackageValidator(translationService, new H5PConfig());
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow(translationService.getTranslation("missing-h5p-extension"));
    })

    it('rejects non-valid zip files', async () => {
        const h5pFile = `${path.resolve('')  }/test/data/validator/corrupt_archive.h5p`
        const validator = new H5pPackageValidator(translationService, new H5PConfig());
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow(translationService.getTranslation("unable-to-unzip"));
    })
    ...
    it('rejects too large total content size', async () => {
        ...
        const validator = new H5pPackageValidator(translationService, config);
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow("The total size of the unpacked files exceeds the maximum size allowed. (<em>9.21 MB</em> &gt; <em>10 Bytes</em>)");
    });
    ...
    it('rejects invalid file extensions', async () => {
        ...
        const validator = new H5pPackageValidator(translationService, config);
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow("File &quot;<em>content/earth.jpg</em>&quot; not allowed. Only files with the following extensions are allowed: <em>json</em>.\nFile &quot;<em>H5P.GreetingCard-1.0/README.md</em>&quot; not allowed. Only files with the following extensions are allowed: <em>json</em>.");
    });

    it('rejects broken json files', async () => {
        const h5pFile1 = `${path.resolve('')  }/test/data/validator/broken-h5p-json.h5p`
        const h5pFile2 = `${path.resolve('')  }/test/data/validator/broken-content-json.h5p`

        const validator = new H5pPackageValidator(translationService, new H5PConfig());
        await expect(validator.validatePackage(h5pFile1)).rejects.toThrow("Unable to parse JSON from the package: <em>h5p.json</em>");
        await expect(validator.validatePackage(h5pFile2)).rejects.toThrow("Unable to parse JSON from the package: <em>content/content.json</em>");
    });

    it('rejects files with h5p.json not conforming to schema', async () => {
        const h5pFile = `${path.resolve('')  }/test/data/validator/invalid-h5p-json-schema.h5p`
        const validator = new H5pPackageValidator(translationService, new H5PConfig());
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow("The main h5p.json does not conform to the schema (<em>should have required property &#39;mainLibrary&#39;</em>)");
    })

    it('rejects files with malformed library directory names', async () => {
        const h5pFile = `${path.resolve('')  }/test/data/validator/malformed-library-directory.h5p`
        const validator = new H5pPackageValidator(translationService, new H5PConfig());
    ...
    it('rejects files with missing preloaded JavaScript files', async () => {
        const h5pFile = `${path.resolve('')  }/test/data/validator/missing-preloaded-js.h5p`
        const validator = new H5pPackageValidator(translationService, new H5PConfig());
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow("The file &quot;<em>greetingcard.js</em>&quot; is missing from library: &quot;<em>H5P.GreetingCard-1.0</em>&quot;");
    })

    it('rejects files with invalid library directory names', async () => {
        const h5pFile = `${path.resolve('')  }/test/data/validator/invalid-library-directory-name.h5p`
        const validator = new H5pPackageValidator(translationService, new H5PConfig());
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow("Library directory name must match machineName or machineName-majorVersion.minorVersion (from library.json). (Directory: <em>H5P.GreetingCard-1.0</em> , machineName: <em>H5P.AnotherLibrary</em>, majorVersion: <em>1</em>, minorVersion: <em>0</em>)");
    })

    it('rejects files with invalid language files', async () => {
        const h5pFile = `${path.resolve('')  }/test/data/validator/invalid-language-file-json.h5p`
        const validator = new H5pPackageValidator(translationService, new H5PConfig());
        await expect(validator.validatePackage(h5pFile)).rejects.toThrow("The JSON of the language file <em>fr.json</em> in the library <em>H5P.GreetingCard-1.0</em> cannot be parsed");
    })
