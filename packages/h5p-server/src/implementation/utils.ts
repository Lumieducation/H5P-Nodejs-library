...
export function generalizedSanitizeFilename(
    ...
    let extension = path.posix.extname(cleanedFilename);
    let basename = path.posix.basename(cleanedFilename, extension);
    const dirname = path.posix.dirname(cleanedFilename);
    if (extension === '') {
        extension = basename;
        basename = 'file';
