...
const {
    ValidationError,
    ValidatorBuilder
} = require('./helpers/validator-builder');
const throwErrorsNow = require('./helpers/validator-builder')
    .throwErrorsNowRule;
...
class H5pPackageValidator {
    ...
    constructor(translationService, configurationValues) {
        ...
        this._libraryExtensionWhitelist = this._config.libraryWhitelist
            .split(' ')
            .concat(this._contentExtensionWhitelist);
    ...
    async validatePackage(h5pFile, checkContent = true, checkLibraries = true) {
        ...
        return new ValidatorBuilder()
            ...
            .addRuleWhen(
                this._fileExtensionMustBeAllowed(
                    name => name.startsWith('content/'),
                    this._contentExtensionWhitelist
                ),
                checkContent
            )
            .addRuleWhen(
                this._fileExtensionMustBeAllowed(
                    ...
                    this._libraryExtensionWhitelist
                ),
                checkLibraries
            )
            .addRuleWhen(
            ...
            .addRuleWhen(
                this._jsonMustConformToSchema(
                    'h5p.json',
                    this._h5pMetadataValidator,
            ...
            .addRuleWhen(
                this._jsonMustBeParsable('content/content.json'),
                checkContent
            )
            .addRule(throwErrorsNow.bind(this))
            .addRuleWhen(
                this._filesMustBeReadable(filePath =>
    ...
    async _validateLibrary(zipEntries, libraryName, error) {
        try {
            return await new ValidatorBuilder()
                ...
                .addRule(
                    this._libraryMustHaveMatchingDirectoryName(libraryName)
                )
    ...
    async _mustHaveH5pExtension(h5pFile, error) {
        if (path.extname(h5pFile).toLocaleLowerCase() !== '.h5p') {
            throw error.addError(
                this._translationService.getTranslation('missing-h5p-extension')
            );
        }
        return h5pFile;
    }
    ...
    async _zipArchiveMustBeValid(h5pFile, error) {
        ...
        if (!zipArchive) {
            throw error.addError(
                this._translationService.getTranslation('unable-to-unzip')
            );
        }
    ...
    async _fileSizeMustBeWithinLimits(zipEntries, error) {
        ...
        if (
            this._config.maxTotalSize &&
            totalFileSize > this._config.maxTotalSize
        ) {
            error.addError(
                this._translationService.getTranslation(
        ...
        return async (zipEntries, error) => {
            for (const zipEntry of zipEntries) {
                ...
                if (
                    ...
                    !H5pPackageValidator._isAllowedFileExtension(
                        lowercaseName,
                        whitelist
                    )
                ) {
    ...
    _jsonMustConformToSchema(
        ...
        return async (zipEntries, error) => {
            ...
            if (!schemaValidator(jsonData)) {
                throw error.addError(
                    this._translationService.getTranslation(errorMessageId, {
                        '%name': entry.fileName,
        ...
        return async (zipEntries, error) => {
            for (const entry of zipEntries.filter(
                e =>
                    ...
                    !H5pPackageValidator._isDirectory(e.fileName)
            )) {
                try {
    ...
    async _librariesMustBeValid(zipEntries, error) {
        const topLevelDirectories = H5pPackageValidator._getTopLevelDirectories(
            zipEntries
        );
        await Promise.all(
            topLevelDirectories
                ...
                .map(directory =>
                    this._validateLibrary(zipEntries, directory, error)
                )
        );
        return zipEntries;
    }
        ...
        if (jsonData.preloadedJs) {
            await Promise.all(
                jsonData.preloadedJs.map(file =>
                    this._fileMustExist(
                        H5pPackageValidator._pathJoin(dirName, file.path),
                        this._translationService.getTranslation(
    ...
    async _libraryLanguageFilesMustBeValid({ zipEntries, jsonData }, error) {
        const dirName = `${jsonData.machineName}-${jsonData.majorVersion}.${jsonData.minorVersion}`;
        const languagePath = H5pPackageValidator._pathJoin(
            dirName,
            'language/'
        );
    ...
    _checkCoreVersion(metadata, libraryName, error) {
        ...
        if (
            ...
            (metadata.coreApi.majorVersion ===
                this._config.coreApiVersion.major &&
                metadata.coreApi.minorVersion >
                    this._config.coreApiVersion.minor)
        ) {
            error.addError(
                this._translationService.getTranslation(
